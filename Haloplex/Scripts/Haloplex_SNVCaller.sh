...
for k in `cat samples.txt`; do
	...
	$java -Xmx40g -jar $LocatIt -X $filepath -U -IS -OB -b $dedup_bed -o ./tempfiles/${k}_RMD ./tempfiles/${k}.sam ${k}_L001_I2_001.fastq.gz
	#Convert bam without duplicates in fastq file
	$java -Xmx40g -jar ${PICARD} SamToFastq I=./tempfiles/${k}_RMD.bam F=./tempfiles/${k}_R1.fastq F2=./tempfiles/${k}_R2.fastq
